...
and trans_var_decl ?(storage = Cil.NoStorage) (scope : Scope.t) fundec loc
  ...
  | Some e -> (
      ...
      | C.Ast.InitList el, Cil.TArray(arr_type, arr_exp, _) ->
          ...
          (* while *)
          let cond_expr = Cil.BinOp (Cil.Lt, tmp_var_expr, Cil.integer arr_len, Cil.intType) in
          ...
          let label = Cil.Label ("asd", loc, false) in
          let break_stmt = Cil.mkStmt (Cil.Break loc) in
          break_stmt.labels <- [ label ];
          let goto_stmt_kind = Cil.Goto (ref break_stmt, loc) in
          let goto_stmt = Cil.mkStmt goto_stmt_kind in
          (* goto_stmt.labels <- [ label ]; *)
          let goto_stmt_block = Cil.mkBlock [ goto_stmt ] in
          let if_stmt = Cil.mkStmt (Cil.If (cond_expr, goto_stmt_block, empty_block, loc)) in 

          (* loop *)
          let block = Cil.mkBlock ([if_stmt] @ [unary_plus_stmt]) in
          let loop_stmtkind = Cil.Loop (block, loc, None, (Some break_stmt)) in
          let loop_stmt = Cil.mkStmt loop_stmtkind in
          (List.iter
            (fun stmt ->
              Printf.printf "%d\n" (List.length stmt.Cil.labels))
          block.bstmts);          
          *)
